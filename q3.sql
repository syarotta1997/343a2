...
CREATE VIEW participation_ratio AS
    select extract(year from e_date) as year, country_id as cid, election.id as eid,
              cast ( (votes_cast+0.0) / electorate as numeric) as ratio
    from election
...
select * from avg_ratio order by year desc;
